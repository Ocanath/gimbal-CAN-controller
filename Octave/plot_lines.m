...
for i = 1:num_lines
    if(i <= length(colorlist))
        anim_line(i) = animatedline('Color', colorlist(i,:), 'LineWidth', 1.5);
...
while(ishandle(H))
    ...
    try
        ...
        %RS485 dongle injects a 0 character at the end for somefucking
        %reason.
         data8 = read(s,nbytes_type*num_lines + 1,'uint8');
         data8 = uint8(data8);
         data8 = data8(1:end-1);
         data = typecast(uint8(data8),type); 
